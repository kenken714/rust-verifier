...
impl<'a, 'tcx> ThirPrinter<'a, 'tcx> {
    ...
    fn print_block(&mut self, block_id: BlockId, depth_lvl: usize) {
        ...
        print_indented!(
            self,
            format!("targeted_by_break: {}", targeted_by_break),
            depth_lvl + 1
        );
        print_indented!(self, format!("span: {:?}", span), depth_lvl + 1);
        print_indented!(
            self,
            format!("region_scope: {:?}", region_scope),
            depth_lvl + 1
        );
        print_indented!(
            self,
            format!("safety_mode: {:?}", safety_mode),
            depth_lvl + 1
        );
    ...
    fn print_expr(&mut self, expr: ExprId, depth_lvl: usize) {
        ...
        print_indented!(
            self,
            format!("temp_lifetime: {:?}", temp_lifetime),
            depth_lvl + 1
        );
        print_indented!(self, format!("span: {:?}", span), depth_lvl + 1);
    ...
    fn print_expr_kind(&mut self, expr_kind: &ExprKind<'tcx>, depth_lvl: usize) {
        ...
        match expr_kind {
            ...
            If {
                ...
                print_indented!(
                    self,
                    format!("if_then_scope: {:?}", if_then_scope),
                    depth_lvl + 1
                );
            ...
            Borrow { borrow_kind, arg } => {
                ...
                print_indented!(
                    self,
                    format!("borrow_kind: {:?}", borrow_kind),
                    depth_lvl + 1
                );
                print_indented!(self, "arg:", depth_lvl + 1);
                self.print_expr(*arg, depth_lvl + 2);
            ...
            ZstLiteral { user_ty } => {
                print_indented!(
                    self,
                    format!("ZstLiteral(user_ty: {:?})", user_ty),
                    depth_lvl
                );
            }
            NamedConst {
    ...
    fn print_adt_expr(&mut self, adt_expr: &AdtExpr<'tcx>, depth_lvl: usize) {
        ...
        print_indented!(
            self,
            format!("user_ty: {:?}", adt_expr.user_ty),
            depth_lvl + 1
        );
    ...
    fn print_adt_def(&mut self, adt_def: ty::AdtDef<'tcx>, depth_lvl: usize) {
        ...
        print_indented!(
            self,
            format!("variants: {:?}", adt_def.variants()),
            depth_lvl + 1
        );
